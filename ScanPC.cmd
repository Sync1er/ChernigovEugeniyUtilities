...
$hide="RAS Async Adapter","WAN Miniport","Virtual Adapter","FAX","Microsoft XPS","OneNote","Корневая очередь печати","PDF","Microsoft Kernel Debug Network Adapter","HTREE"
$repl="Intel(R) Core(TM) ","11th Gen Intel(R) Core(TM) ","Intel(R) ","System Product Name"," 82371AB/EB PCI Bus Master"," To be filled by O.E.M."," Virtual Platform"," Express Chipset Family"," Express Chipset","\(корпорация Майкрософт - WDDM 1.1\)","Набор микросхем ","Семейство наборов микросхем ","/C200 Series 6 Port","6th Generation Core Processor Family Platform "," Family Controller","APU with Radeon(tm) HD Graphics"," Storage Controllers"," Universal Host Controller","\(Майкрософт\)","Series/C200 Series Chipset Family USB ","Корневой ","Последовательный порт ","Расширенный ","Расширяемый ","Стандартный "," Enhanced Host Controller"," Storage Controller"," Chipset Family"," LaserJet Professional"
$sprtr="-------------------------------------------------"
$list= @()
$SlctnStrt= 0
$global:usrSz= @()
...
function Chek {
  ...
  if($dt){
    ...
    $tmUp=$curTm - $dt
    if($dt){Output " System boot up date        Uptime`r`n --------------------     -----------`r`n $($dt.ToString('dd MMM yyyy HH:mm'))         $($tmUp.Days)d $($tmUp.Hours):$("{0:d2}" -f $tmUp.Minutes)`r`n`r`n"}
  }
