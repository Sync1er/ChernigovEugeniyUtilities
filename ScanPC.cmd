...
# Chernigov Eugeniy 2024 or non commercial use only
...
$timer.add_tick({
  ...
  if($step -eq 4.5){    
    if(Get-Job | Where {$_.Name -eq 'System files'}){
      ...
      if(-not(Get-Job | Where { $_.Name -eq 'System files'})){
        $global:step=5
        if($os -and $CheckBox2.Checked){Output "`nScan for Windows temp and users folders size more than 100MB... `n`n"}
      }
    }else{$global:step=5}
  }
  ...
  if($step -eq 4.1){    
    if(Get-Job | Where {$_.Name -eq 'System files Invoke'}){
      ...
      if($out){Output $out; $global:step=4.2; ShCRButton $out; Output "`n`nScan for Windows temp and users folders size more than 100MB... `n`n"}
  ...
  if($step -eq 2){
    ...
    if($job.count -eq (Get-Job | Where { $_.Name -eq 'Get Printers Invoke'}).count){
      $global:printTabl= $($job | Receive-Job)
      $job | Remove-Job -Force
      if($printTabl){$printrs= PrintTabl $printTabl}else{GetPrinters;$global:step=2.2}
      if($printrs){PrintersOut $printrs}
      $global:step=2.5
    }
  }
...
function Chek {
  ...
  if(-not $global:compAdr){$global:compAdr=$env:COMPUTERNAME}
  $ListBox3.text="`r`n"
  $ip= GetIP $compAdr
...
function CompHw {
  ...
  if ($ip){
    $reg = 'LocalMachine','SOFTWARE\Microsoft\Virtual Machine\Guest\Parameters','HostName'
    $hst=$([Microsoft.Win32.RegistryKey]::OpenRemoteBaseKey($reg[0], $compAdr).OpenSubKey($reg[1], $true).GetValue($reg[2])) 2>$nul
...
function GetPrintersInvoke {  
  ...
  Start-Job -InputObject "$compAdr" -Name 'Get Printers Invoke' -ScriptBlock { 
    ...
    .$gtReg | ?{$_ -notlike "*:OneNote*"} | %{$name=$_;$pth2="$_\$vl"; $prnr= .$gtReg; @{Class=$class; Name=$name; Value=$prnr}}
    }
  }
}
